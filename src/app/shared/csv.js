(function(root, factory) {
  ...
  let CSV = (function() {
    function CSV(data, options) {
      ...
      let lineDelimiter = options.lineDelimiter || options.line,
    ...
    CSV.prototype.encode = function(callback) {
      ...
      let recordType = serializeType(sample),
          map;
      ...
      if (recordType == 'array') {
        ...
        forEach(data, function(record, recordIndex) {
          let serializedRecord = Array(map.length);
        ...
        forEach(data, function(record, recordIndex) {
          let serializedRecord = Array(attributes.length);
